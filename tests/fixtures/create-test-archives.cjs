...
fs.writeFileSync(path.join(TEST_FILES_DIR, 'readme.md'), '# Test Archive\n\nThis is a test file for archive testing.\n');
fs.writeFileSync(path.join(TEST_FILES_DIR, 'data.json'), JSON.stringify({ test: true, value: 42 }, null, 2));
...
async function createTarBz2() {
  try {
    ...
    execSync(`bzip2 -k "${path.join(OUTPUT_DIR, 'test.tar')}"`, { stdio: 'inherit' });
    fs.renameSync(path.join(OUTPUT_DIR, 'test.tar.bz2'), path.join(OUTPUT_DIR, 'test.tar.bz2'));
