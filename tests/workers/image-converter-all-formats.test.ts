...
describe('Image Converter - All Formats Comprehensive Test', () => {
  ...
  describe('Reading generated formats', () => {
    const formats = [
      ...
      { file: 'test.avif', mime: 'image/avif', name: 'AVIF', skipMetadata: true },
      { file: 'test.ff', mime: 'image/farbfeld', name: 'Farbfeld', skipMetadata: true },
    ];
  ...
  describe('SVG decoding', () => {
    it('should decode SVG to raster format', () => {
      ...
      if (existsSync(svgPath)) {
        ...
        // Convert SVG to PNG
        const pngResult = convertImage(svgData, 'image/svg+xml', 'image/png', () => {});
    ...
    it('should not be able to encode to SVG', () => {
      const pngData = new Uint8Array(readFileSync(join(__dirname, 'fixtures/images/test.png')));
  ...
  describe('Format-specific features', () => {
    it('should handle ICO size constraints', () => {
      const pngData = new Uint8Array(readFileSync(join(__dirname, 'fixtures/images/test.png')));
      const icoResult = convertImage(pngData, 'image/png', 'image/x-icon', () => {});
    ...
    it('should handle formats without alpha channel', () => {
      ...
      const transparentPng = new Uint8Array(readFileSync(join(__dirname, 'fixtures/images/transparent.png')));
      
      const jpegResult = convertImage(transparentPng, 'image/png', 'image/jpeg', () => {});
    ...
    it('should handle high dynamic range formats', () => {
      ...
      // Convert to HDR
      const hdrResult = convertImage(pngData, 'image/png', 'image/vnd.radiance', () => {});
      expect(hdrResult).toBeInstanceOf(Uint8Array);
      
      // Convert to EXR
      const exrResult = convertImage(pngData, 'image/png', 'image/x-exr', () => {});
  ...
  describe('Cross-format conversions', () => {
    const testConversions = [
      { from: 'TIFF', fromMime: 'image/tiff', to: 'JPEG', toMime: 'image/jpeg' },
      ...
      { from: 'TGA', fromMime: 'image/x-targa', to: 'JPEG', toMime: 'image/jpeg' },
    ];

    testConversions.forEach(({ from, fromMime, to, toMime }) => {
      it(`should convert ${from} to ${to}`, () => {
        const sourceFile = join(__dirname, `fixtures/images/generated/test.${from.toLowerCase()}`);
  ...
  describe('Edge cases', () => {
    it('should handle very small images (1x1)', () => {
      ...
      if (existsSync(pixelPath)) {
        ...
        const jpegResult = convertImage(data, 'image/png', 'image/jpeg', () => {});
        expect(jpegResult).toBeInstanceOf(Uint8Array);
      }
    });
