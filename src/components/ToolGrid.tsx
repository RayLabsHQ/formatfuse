...
export default function ToolGridNew() {
  ...
  const [hoveredPopularTool, setHoveredPopularTool] = useState<string | null>(null);
    ...
    <section id="all-tools" className="relative py-20 overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none" aria-hidden="true">
        <div className="absolute top-1/3 left-0 w-96 h-96 rounded-full opacity-10 blur-3xl"
          style={{ background: `radial-gradient(circle, var(--gradient-1), transparent)` }} />
        <div className="absolute bottom-1/3 right-0 w-96 h-96 rounded-full opacity-10 blur-3xl"
          style={{ background: `radial-gradient(circle, var(--gradient-2), transparent)` }} />
      </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className={`text-center mb-16 space-y-6 transition-all duration-700 ${
          ...
          <p className="text-lg md:text-xl text-muted-foreground max-w-2xl mx-auto">
            Professional-grade file conversion tools that work instantly in your browser. 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {popularTools.map((tool, index) => {
                ...
                return (
                  ...
                  >
                    <div className={`relative h-full p-6 rounded-2xl bg-card/50 backdrop-blur-sm border border-border/50 transition-all duration-300 ${
                      isHovered ? 'transform -translate-y-1 shadow-lg border-primary/50' : ''
                    }`}>
        ...
        {/* All tools grid */}
        <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 transition-all duration-700 delay-300 ${
          isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
        }`}>
          {filteredTools.map((tool, index) => {
            ...
            return (
              ...
              >
                <div className={`relative h-full p-5 rounded-xl bg-card/30 backdrop-blur-sm border border-border/30 transition-all duration-300 ${
                  isHovered ? 'transform -translate-y-1 shadow-md border-primary/30 bg-card/50' : ''
                }`}>
                  <div className="relative">
                    ...
                    {/* Hover indicator */}
                    <div className={`absolute -right-2 -top-2 w-2 h-2 rounded-full transition-all duration-300 ${
