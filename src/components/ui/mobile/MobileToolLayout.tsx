  ...
  return (
    <header
      className={cn(
        ...
        "px-4 py-3",
        sticky &&
          "sticky top-0 z-40 lg:relative lg:bg-transparent lg:backdrop-blur-none lg:border-0",
      )}
    >
      <div className="flex items-center justify-between gap-4">
        <div className="flex-1 min-w-0">
          <h2 className="text-lg font-semibold truncate lg:text-2xl">
            {title}
          </h2>
          {description && (
        ...
        {action && <div className="flex-shrink-0 lg:hidden">{action}</div>}
      </div>
    </header>
  );
}
...
export function MobileToolFooter({
  children,
  className,
}: MobileToolFooterProps) {
  return (
    <footer
      className={cn(
