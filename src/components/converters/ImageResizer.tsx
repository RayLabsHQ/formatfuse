  ...
  [
    {
      value: "stretch",
      label: "Stretch",
      description: "Stretch to exact dimensions",
    },
    {
      value: "contain",
      label: "Contain",
      description: "Fit within dimensions",
    },
...
const faqs: FAQItem[] = [
  {
    ...
    answer:
      "Each method uses different algorithms: Lanczos provides the highest quality for photos, Mitchell offers a good balance, Catmull-Rom preserves sharp details, Triangle is fastest, and Magic Kernel uses advanced AI-inspired techniques.",
  },
  {
    question: "How do fit methods work?",
    answer:
      "Stretch distorts the image to match exact dimensions. Contain scales down to fit within bounds. Cover scales up to fill the area. Fill crops to exact dimensions while maintaining aspect ratio.",
  },
  {
    question: "Can I resize multiple images at once?",
    answer:
      "Yes! You can select or drag multiple files for batch resizing. All images will be processed with the same settings and can be downloaded individually or as a ZIP file.",
  },
  {
    question: "What's the maximum image size supported?",
    answer:
      "We can handle images up to 50MB each. For best performance, we recommend keeping individual files under 20MB. There's no limit on the number of files you can process.",
  },
];

export default function ImageResizer() {
  const { resizeBatch, isResizing, batchProgress, error, clearError } =
    useImageResize();
  ...
  React.useEffect(() => {
    if (batchProgress.length > 0) {
      setFiles((prev) => {
        const updated = [...prev];
        const processingFiles = updated.filter(
  ...
  return (
    <div className="w-full">
      {/* Subtle Gradient Blobs - Hidden on mobile */}
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setOptions((prev) => ({
                            ...prev,
                            method: e.target.value as ResizeMethod,
                          }))
                        }
                      ...
                      <p className="text-xs text-muted-foreground mt-1">
                        {
                          RESIZE_METHODS.find((m) => m.value === options.method)
                            ?.description
                        }
                      </p>
                    </div>
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setOptions((prev) => ({
                            ...prev,
                            fitMethod: e.target.value as FitMethod,
                          }))
                        }
                        className="w-full px-3 py-2.5 border border-border rounded-lg bg-background text-sm"
                      >
                      ...
                      <p className="text-xs text-muted-foreground mt-1">
                        {
                          FIT_METHODS.find((m) => m.value === options.fitMethod)
                            ?.description
                        }
                      </p>
                    </div>
