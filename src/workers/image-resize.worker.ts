...
export type ResizeMethod = 'triangle' | 'catrom' | 'mitchell' | 'lanczos3' | 'hqx' | 'magicKernel' | 'magicKernelSharp2013' | 'magicKernelSharp2021';
...
class ImageResizeWorker {
  ...
  private async encodeImage(imageData: ImageData, format: ImageFormat, quality: number = 0.85): Promise<Blob> {
    if (typeof OffscreenCanvas !== 'undefined') {
  ...
  async resizeImage(
    ...
    try {
      ...
      const resizedImageData = await resize(imageData, {
        ...
        fitMethod: options.fitMethod === 'fill' ? 'stretch' : (options.fitMethod || 'stretch'),
        premultiply: true,
        linearRGB: true
      });
      ...
      const format = options.format || blob.type.split('/')[1] as ImageFormat || 'jpeg';
      const outputBlob = await this.encodeImage(resizedImageData, format, options.quality);
