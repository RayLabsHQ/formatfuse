...
class PDFOperationsWorker {
  ...
  async rotate(
    ...
    const pdfDoc = await this.loadPdfDocument(pdfData);
    const totalPages = pdfDoc.getPageCount();
    const pagesToRotate = options.pages?.map(p => p - 1) || Array.from({ length: totalPages }, (_, i) => i);
  ...
  async extract(
    ...
    const pageIndices = options.pages.map(p => p - 1).filter(idx => idx >= 0 && idx < pdfDoc.getPageCount());
  ...
  async pdfToImages(
    ...
    const pagesToConvert = options.pages || Array.from({ length: numPages }, (_, i) => i + 1);
