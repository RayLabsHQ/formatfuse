...
async function convertJpgToPdf(images: Array<{ data: ArrayBuffer; name: string }>): Promise<ArrayBuffer> {
  try {
    ...
    for (const imageData of images) {
      ...
      self.postMessage({ type: 'progress', progress: Math.round(progress) } as ProgressMessage);
    }
